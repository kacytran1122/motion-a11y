...
function stats(values) {
  ...
  return { mean: sum / sorted.length, p50: at(50), p95: at(95), min: sorted[0], max: sorted[sorted.length - 1] };
}

function peakRss() { return process.memoryUsage().rss; }
...
const report = { label, node: process.version, throughput: {}, scaling: [], memory: {}, correctness: {} };
...
function measureCorpus(options) {
  const run = () => {
    let messages = 0;
    for (const f of corpus) messages += lint(f.code, { filename: f.filename, ...options }).messages.length;
    return messages;
  };
...
for (const bulk of [1, 4, 16, 64, 256]) {
  ...
  const run = () => { for (const f of files) lint(f.code, { filename: f.filename, prefilter: false }); };
...
const hugeSamples = stats(time(() => lint(huge.code, { filename: huge.filename, prefilter: false }), 5, 1));
report.largeFile = {
